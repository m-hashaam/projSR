...
$result = $db->query("select firstname, lastname, userid, mobilenumber,email from `profile`");
while($row = $result->fetch_assoc()) {
	$temp = $temp+1;
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('A'.$temp, $row["userid"]);
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('B'.$temp, $row["firstname"]);
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('C'.$temp, $row["lastname"]);
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('D'.$temp, $row["mobilenumber"]);
	$objPHPExcel->setActiveSheetIndex(0)->setCellValue('E'.$temp, $row["email"]);
}



// Rename worksheet
$objPHPExcel->getActiveSheet()->setTitle('AppUsers');
